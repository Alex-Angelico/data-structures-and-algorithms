...
function getCurrentEvents(request, response) {
  // Solution code here...
  let newsDisplay = mapCurrentEvents();
  response.json(newsDisplay);
}

const mapCurrentEvents = () => {
  // Solution code here...
  let newsData = currentEvents.news;
  let newsArticles = newsData.map(object => new Event(object));
  return newsArticles;
}
...
function Event(obj) {
  ...
  this.categories = obj.category.map(value => value);
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let countArr = arr.reduce(function (accumulator, value) {
...
const returnNames = (arr) => {
  // Solution code here...
  let characterArray = [];
  let swCharacterNames = arr.reduce(function (swCharacters, character) {
    characterArray.push(character.name);
    return characterArray;
  }, 0);
  return swCharacterNames;
};
...
const reversedString = (str) => {
  ...
  let reverseString = stringArray.reduce(function (stringReversed, letter) {
    stringReversed = `${letter}${stringReversed}`;
