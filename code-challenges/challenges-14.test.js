...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let biggerChars = '';
  arr.reduce((acc, curr, i, old) => {
    curr.mass = Number(curr.mass);
    if (curr.mass > Number(old[0].mass)) { biggerChars += `${curr.name} - `; }
  }, '')
  biggerChars = biggerChars.substring(0, biggerChars.length - 3);
  return biggerChars;
};
