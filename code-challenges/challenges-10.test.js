...
const createServer = () => {
  ...
  app.get('/favoritefoods', handleFoodFaves);
  ...
  function handleBio(req, res) {
    res.send({ 'msg': 'My name is Alex Angelico. I\'m a software developer at Code Fellows, and I live in Seattle, Washington.' });
  }

  function handleFoodFaves(req, res) {
    res.send(['Fish head curry', 'Thai pork', 'Beef Wellington', 'Fresh sushi', 'Dick\'s cheeseburgers']);
  }
...
const count = (target, input) => {
  // Solution code here...
  let intCount = input.reduce((acc, curr) => {
...
const totalSum = (input) => {
  // Solution code here...
  let arrTotal = input.reduce((acc, curr) => {
    for (let i = 0; i < curr.length; i++) {
      acc += curr[i];
    }
    return acc;
