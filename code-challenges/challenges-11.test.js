...
const toLastNames = people => {
  // Solution code here...
  let peopleNames = people.map(object => { return `${object.firstName} ${object.lastName}`; })
  return peopleNames;
};
...
const validateEmail = (email) => {
  // Solution code here...
  let regex = /^([a-zA-Z0-9]+[@]|[a-zA-Z0-9]+\.[a-zA-Z0-9]+[@])[a-zA-Z0-9]+\.((com)|(net)|(org))$/g;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let regex = /^(\d{3}|\({1}\d{3}\){1}) ?-?\d{3} ?-?\d{4}$/g;
